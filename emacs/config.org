...
* Early Init
These settings MUST be evaluated before any other source block, because
several =use-package= forms further down (e.g. =eca=, =eldoc-box=) eagerly
load and call =evil-leader/set-key=, which in turn =(require 'evil)= via
=evil-leader=. If =evil-want-keybinding= is still at its default value =t=
when that happens, evil will pull in =evil-keybindings=, and later
=evil-collection-init= will print the warning:

#+begin_quote
evil-want-keybinding was set to nil but not before loading evil.
#+end_quote

Setting these here, in the very first tangled block, guarantees they are
in effect before any package can transitively load evil.
...
*** Indent Bars
Visualise indentation with subtle vertical bars in =prog-mode= and YAML
buffers. Uses the stipple-based renderer (requires PGTK/X11 Emacs ≥28)
and tree-sitter awareness so blank lines and structural nodes are
handled correctly.
...
*** Eca
ECA (Editor Code Assistant) is an AI-powered agentic pair-programming
client for Emacs. Unlike copilot-chat which focuses on conversations
(explain, review, fix), ECA is a full agentic assistant that can read
and write files, execute commands, and perform multi-step tasks
autonomously. It connects to an external server process via JSONRPC
and supports MCP (Model Context Protocol), inline code completion,
and code rewriting. The server binary is automatically downloaded
on first use.

Usage:
- Run =M-x eca= to start a session in your project directory.
- Type your prompt after =>= and press =RET= to send.
- Use =@= to attach files/dirs as context.
- Press =C-c .= in the ECA buffer for the transient command menu.
- Tool calls (file writes, commands) require your approval.
...
*** Dtrt Indent
Auto-detect the indentation offset and tabs-vs-spaces style of the file
you are visiting. Saves having to fiddle with =indent-tabs-mode= or
=*-indent-offset= per project — the buffer adapts to whatever the
existing code uses. Plays nicely with =editorconfig= (editorconfig wins
...
By default ~clean-buffer-list-kill-regexps~ is ~nil~, which means
*nothing* is ever a candidate for cleanup — that's why midnight
appeared to do nothing. We opt in to "kill anything" by setting it to
~'(".*")~ and rely on ~clean-buffer-list-kill-never-regexps~ below to
protect important buffers (scratch, messages, dashboard, tramp, mu4e,
magit, ...). midnight only kills buffers that have been *idle* for at
least ~clean-buffer-list-delay-general/-special~ seconds, so the
buffer you are actively working in is always safe.

For an *on-demand* "kill all buffers except the current one" (including
vterm/term/process buffers), use ~SPC k a~ → ~kill-other-buffers~.
...
Generic, plugin-agnostic helpers and bindings. Anything that depends on
a specific plugin (consult, projectile, vterm, ...) lives next to that
plugin's =use-package= block so removing the plugin removes its
keybindings with it.
...
*** Java
=lsp-java= drives the JDT language server (refactorings, code actions,
imports, ...). Debugging is provided by =dap-java= (which now ships
inside =lsp-java= itself, see [[*DAP][DAP]]); the test/main-class debug
entry points live alongside the rest of the Java leader bindings under
=SPC l j=.
...
*** Postgres / MySQL
SQL tooling. Three complementary entry points:

- =pgmacs= — a buffer-based browser/editor that talks the Postgres wire
  protocol directly via =pg.el= (no =psql= required). Connect once and
  navigate tables, edit rows, follow foreign keys, export CSV/JSON.
- =sql-postgres= — the classic =sql.el= comint REPL backed by =psql= for
  ad-hoc query sessions and shipping regions/paragraphs from a =.sql=
  buffer.
- =sql-connect= — pick a named connection from =sql-connection-alist=.
  Two local entries are defined: =postgres-local= (5432) and
  =mysql-local= (3306). Password is prompted on connect; nothing is
  stored on disk. To suppress the prompt, add a matching line to
  =~/.authinfo= or =~/.pgpass= / =~/.my.cnf= as appropriate.

All bindings live under the =SPC D= leader prefix (capital D — lowercase
=d= is taken by =detached=).
...
*** Tailwind CSS
Tailwind CSS language server, runs as an =lsp-mode= add-on alongside the
primary LSP (volar/ts-ls/etc.) so class-name completion, hover docs and
linting work in the same buffer. Only activates in projects that have a
...
*** LSP DAP
=dap-mode= adds Debug Adapter Protocol support on top of =lsp-mode=:
breakpoints, stepping, watch expressions, a REPL and a debug UI. We
enable it for Java (via =dap-java=, which now ships inside =lsp-java=
and drives the JDT language server's debug plugin) and Scala (via
=lsp-metals=, which registers Scala debug templates and talks to
Metals' BSP-based DAP server — works for both =sbt= and =bloop=
projects, no separate =dap-scala= package required).

All generic debug bindings live under the =SPC b= leader prefix
(=b= for *break/debug* — =SPC d= is taken by =detached=). Language
specific helpers (=dap-java-*=) are bound next to the rest of the
Java tooling, in the [[*Java][Java]] section.
...
*** Dumb Jump
Heuristic, language-agnostic jump-to-definition powered by ripgrep. Hooks
into =xref= as a fallback backend so the regular =M-.= / =gd= bindings
keep working: when an LSP server is active it answers, and when one is
not (shell scripts, scratch files, languages without a server)
=dumb-jump= takes over via grep heuristics. The leader binding under
=j= exposes a transient menu listing all jump targets.
...
Setup emacs-vterm
- Emulate a Terminal
- Using Posframe to spawn a project floating terminal
...
** Version
Using magit with forge plus =pr-review= creates a complete environment
for working with Git inside Emacs: forge for managing repos and issues,
and pr-review for actually reviewing/approving Pull Requests.

For forge and pr-review, create a file called ~/.authinfo with the
following lines (each tool uses its own token suffix):

- machine api.github.com login gako358^forge password ==secret-token==
- machine api.github.com login gako358^emacs-pr-review password ==secret-token==

Both tokens need =repo= and =notifications= scope. You can reuse the same
GitHub PAT for both lines if you want — only the suffix after =^= matters
to ghub.
...
*** Diff-hl
Highlight uncommitted VC changes in the fringe (or the margin in
terminal frames). Works alongside Magit — refreshing Magit also
refreshes the diff indicators — and supports staging hunks directly
from a source buffer with =diff-hl-stage-current-hunk=.
...
*** PR Review
Review GitHub (and GitLab) Pull Requests directly inside Emacs. Forge
is great for browsing issues and PRs, but it cannot actually submit a
review / approve / merge — that's what =pr-review= is for.

Two ways I use it:

1. =SPC P n= (or =:prn=) opens the *PrReviewNotification* dashboard
   listing all GitHub notifications; press =RET= on a PR to review it.
2. From a =mu4e= message buffer showing a GitHub notification email,
   press =C-c C-r= to jump straight into the PR. =pr-review= scans the
   current buffer for an URL — both =thing-at-point= (plain text) and
   the =shr-url= text property (HTML rendered by =shr=, which =mu4e=
   uses for HTML mails) are supported, so the notmuch workflow
   described in the upstream README works identically here.
...
*** Consult-GH
GitHub-wide search and discovery layer on top of the =gh= CLI. Fills
the gap between =magit= (local git), =forge= (per-repo issues/PRs you
have already pulled) and =pr-review= (review/approve/merge a known
PR):

- =SPC G r= search any repository on GitHub
- =SPC G p= search pull requests across GitHub
- =SPC G i= search issues
- =SPC G c= search code (jumps directly to the matching line)
- =SPC G o= browse repositories of one or more users/orgs
- =SPC G G= open the consult-gh transient menu

Issues open in a Magit/Forge buffer (familiar UX), and PRs are routed
through =pr-review= so you can actually review/approve/merge directly.
=gh= handles authentication, so no extra token in =~/.authinfo= is
required for consult-gh itself.
