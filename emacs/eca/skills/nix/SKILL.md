...
- For supported operations, prefer the typed MCP tools: `nix__flake_metadata` and `nix__flake_show` for metadata/show, `nix__flake_check` for checks, `nix__eval` for one specific attribute, and `nix__build` for specific attributes. Pass the authorized `root` and exact `attribute` or `attributes` schema fields, and record their JSON text result as evidence.
- Use shell only for unsupported operations, commands exceeding MCP constraints or timeouts, or MCP debugging. Never retry a policy-rejected expression or path through shell. Preserve `--no-write-lock-file` safety for shell fallback and require recorded MCP or literal shell evidence before claiming success.
